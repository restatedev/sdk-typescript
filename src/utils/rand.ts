...
export class RandImpl implements Rand {
  ...
  u64(): bigint {
    this.randstate64 = wrapping_add(this.randstate64, 0x9e3779b97f4a7c15n)
    let next: bigint = this.randstate64
    next = wrapping_mul((next ^ (next >> 30n)), 0xbf58476d1ce4e5b9n);
    next = wrapping_mul((next ^ (next >> 27n)), 0x94d049bb133111ebn);
    next = next ^ (next >> 31n);
    return next
  }
...
const UINT64_MAX: bigint = 2n**64n
function wrapping_add(a: bigint, b: bigint) {
  return (a + b) % UINT64_MAX
}
function wrapping_mul(a: bigint, b: bigint) {
  return (a * b) % UINT64_MAX
}
