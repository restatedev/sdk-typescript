...
export const serviceInterpreterHelper = service({
  ...
  handlers: {
    ...
    *echoLater(req: { sleep: number; parameter: string }) {
    ...
    *incrementIndirectly(param: { layer: 0 | 1 | 2; key: string }) {
    ...
    *resolveAwakeable(aid: string) {
      resolveAwakeable(aid, "ok");
    },

    *rejectAwakeable(aid: string) {
      rejectAwakeable(aid, "error");
    },

    *incrementViaAwakeableDance(input: {
      txPromiseId: string;
...
function makeInterpretHandler(layer: 0 | 1 | 2) {
  return function* (program: Program): Generator<unknown, void, unknown> {
    const coros = new Map<number, Coro>();

    function* awaitPromise(index: number): Generator<unknown, void, unknown> {
    ...
    function* interpretOne(
      ...
      switch (cmd.kind) {
        ...
        case CALL_SERVICE: {
          ...
          const future = call<string, string>({
        ...
        case CALL_SLOW_SERVICE: {
          ...
          const future = call<typeof arg, string>({
            service: "ServiceInterpreterHelper",
            method: "echoLater",
            parameter: arg,
            inputSerde: restate.serde.json,
        ...
        case INCREMENT_VIA_DELAYED_CALL: {
          const arg = { layer, key: handlerRequest().key! };
          send({
            service: "ServiceInterpreterHelper",
        ...
        case RESOLVE_AWAKEABLE: {
          const { id, promise } = awakeable<string>();
          coros.set(i, { expected: "ok", future: promise, deserialize: false });
          send({
            service: "ServiceInterpreterHelper",
        ...
        case INCREMENT_STATE_COUNTER_VIA_AWAKEABLE: {
          ...
          const arg = {
            interpreter: { layer, key: handlerRequest().key! },
            txPromiseId: txAid,
          };
          send({
            service: "ServiceInterpreterHelper",
        ...
        case CALL_NEXT_LAYER_OBJECT: {
          ...
          const future = call<Program, void>({
            service: nextLayer,
            method: "interpret",
        ...
        default:
          throw new restate.TerminalError(`Unknown command type: ${cmd.kind}`);
      }
    }
...
export const objectInterpreterL0 = object({
  ...
  options: { handlers: { counter: { shared: true } } },
});

export const objectInterpreterL1 = object({
...
export const objectInterpreterL2 = object({
