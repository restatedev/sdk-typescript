...
const channelsSvc = restate.service({
  name: "channels",
  handlers: {
    ...
    basicSend: async (ctx: restate.Context, value: string): Promise<string> =>
      execute(
        ctx,
        gen(function* () {
...
describe.each(modes)("channels — $name mode", ({ alwaysReplay }) => {
  ...
  test("basic send/receive in the same fiber", async () => {
    const client = ingress.serviceClient(channelsSvc);
