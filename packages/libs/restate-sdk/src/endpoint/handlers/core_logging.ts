...
import { LogLevel } from "./vm/types.js";
...
export function vm_log(
  level: LogLevel,
  strBytes: Uint8Array | string,
  loggerId?: number
) {
  try {
    const logger = (loggerId && invocationLoggers.get(loggerId)) || undefined;
    const str =
      typeof strBytes === "string"
        ? strBytes
        : logsTextDecoder.decode(strBytes);
    if (logger !== undefined) {
...
function wasmLogLevelToRestateLogLevel(level: LogLevel): RestateLogLevel {
  switch (level) {
    case LogLevel.TRACE:
